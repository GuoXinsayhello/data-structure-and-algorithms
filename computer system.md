...
动态存储器分配器维护一个进程的虚拟存储器区域，称为堆，向上增长（地址 更高）。 分配器 分为显示分配器与隐式分配器两种，隐式分配器又叫垃圾收集器。字是4字节的对象。碎片有两种 形式：内部碎片和外部碎片。内部碎片是在一个已分配的块比有效载荷大时发生的。外部碎片是当空闲 存储器合计起来能够满足要求，但是没有一个单独的空闲块足够大可以来处理这个请求。<br>
2016/11/7 看到565页<br>
