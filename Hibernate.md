...
并发事务可能出现的问题：<br>
1:第一类丢失更新 2.脏读(读到了第二个事务还没有提交的数据）3，不可重复读（就是读取事务 读取了两遍，结果不一样）4第二类丢失更新（不可重复读的特殊 情况）5，幻读（讨论的是插入和删除，与不可重复读类似）<br>
为了解决这些问题，可以设定数据库的隔离机制，也可以设定HIbernate的事务隔离机制,有四种隔离机制，read uncommitted | 1，read committed | 2，repeatable read | 4，serializable | 8每种隔离机制有自己的并发效率，但是能够解决的问题的能力也不同， 方法的并发 性越强，解决问题的能力越低。用悲观锁解决repeatable read的问题。<br>
为了考虑并发的效率，设置成read_committed,为了解决这种隔离机制下的重复读的问题，就用乐观锁和悲观锁辅助解决。悲观锁就是在事务中上把锁，当自己事务没有完成时，任何人都不能对我的对象进行更改.通过设置 Lockmode对session进行加锁session.lock( person, LockMode.NONE );这个是不加锁。`乐观锁`，大多是基于数据版本（ Version ）记录机制实现。何谓数据版本？即为数据增加一个版本标识，在基于数据库表的版本解决方案中，一般是通过为数据库表增加一个 “version” 字段来实现。读取出数据时，将此版本号一同读出，之后更新时，对此版本号加一。此时，将提交数据的版本数据与数据库表对应记录的当前版本信息进行比对，如果提交的数据版本号大于数据库表当前版本号，则予以更新，否则认为是过期数据。在类的版本号属性上加上@Version,当有session对它更新的时候就会自动加1
